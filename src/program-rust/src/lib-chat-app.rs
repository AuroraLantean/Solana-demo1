...
pub fn process_instruction(
    ...
    let instruction_data_decoded = ChatMessage::try_from_slice(instruction_data).map_err(|err| {
        msg!("Attempt to deserialize instruction data has failed. {:?}", err);
        ProgramError::InvalidInstructionData
    })?;
    msg!("Instruction_data_decoded: {:?}", instruction_data_decoded);

    let mut saved_data_decoded = match <Vec<ChatMessage>>::try_from_slice(&account.data.borrow_mut()) {
        Ok(data) => data,
        Err(err) => {
    ...
    let index = saved_data_decoded.iter().position(|p| p.archive_id == String::from(DUMMY_TX_ID)).unwrap(); // find first dummy data entry
    msg!("Found index {}", index);
    saved_data_decoded[index] = instruction_data_decoded; // set dummy data to new entry
    let saved_data_encoded = saved_data_decoded.try_to_vec().expect("Failed to encode data."); // set messages object back to vector data
    msg!("Final saved_data_decoded[index] {:?}", saved_data_decoded[index]);
